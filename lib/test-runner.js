...
class TestRunner {
  ...
  async runTests() {
    ...
    // wait for ark_node0 to be up + node_modules to exist in dist/test1/node0/node_modules
    await this.waitForNode0Docker(120)
  ...
  async launchNodes() {
    for (let nodeNumber in this.nodes) {
      ...
      const commandLaunch = `docker ps --format "{{.Names}}" | grep ${nodeInfos.name}_ark | xargs -I {} sh -c 'docker exec -d {} bash -c "ARK_LOG_FILE=ark.log packages/core/bin/ark start --config packages/core/lib/config/e2enet --network e2enet ${networkStart}>> output.log"'`
      console.log(`Executing: ${commandLaunch}`)
      await exec(commandLaunch)

      const commandTest = `docker ps --format "{{.Names}}" | grep ${nodeInfos.name}_ark | xargs -I {} sh -c 'docker exec -d {} bash -c "pwd >> output.log"'`
      console.log(`Executing: ${commandTest}`)
      await exec(commandTest)
  ...
  async waitForNode0Docker(retryCount) {
    ...
    try {
      ...
      const commandNodeModuleExists = `ls ${this.rootPath}/dist/${this.network}/node0 | grep node_modules | wc -l`
  ...
  async waitForNode(nodeNumber, retryCount) {
    ...
    try {
      const { stdout, stderr } = await exec(`ls ${this.rootPath}/dist/${this.network}/node0`)
      console.log(stdout)
