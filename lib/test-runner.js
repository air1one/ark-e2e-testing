...
class TestRunner {
  ...
  async runTests() {
    ...
    await this.waitForNode0Docker(480)
  ...
  async waitForNode0Docker(retryCount) {
    ...
    const commandNodeList = `ls ${this.rootPath}/dist/${this.network}/node0`
    const { stdout, stderr } = await exec(commandNodeList)
    console.log(commandNodeList)
    console.log(stdout)
    console.log(stderr)
