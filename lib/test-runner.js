...
class TestRunner {
  ...
  async waitForNode0Docker(retryCount) {
    ...
    const commandNodeModuleExists = `ls ${this.rootPath}/dist/${this.network}/node0`
    const { stdout, stderr } = await exec(commandNodeModuleExists)
    console.log(commandNodeModuleExists)
    console.log(stdout)
    console.log(stderr)
