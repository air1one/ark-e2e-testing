...
class TestRunner {
  ...
  async launchNodes() {
    for (let nodeNumber in this.nodes) {
      ...
      const commandLaunch = `docker ps --format "{{.Names}}" | grep ${nodeInfos.name}_ark | xargs -I {} sh -c 'docker exec -d -w packages/core {} bash -c "ARK_LOG_FILE=ark.log bin/ark start --config lib/config/e2enet --network e2enet ${networkStart}>> output.log"'`
